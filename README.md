...
- **/Code**: contains all of the codebase of this project.
  ...
  - **/Code/parameter_sweep.py**: does a paramater sweep with the values set in the file.
...
The /Code folder also includes some ipython notebooks that explain the code and show how to perform analysis methods on the data that is generated:
...
- **/Code/ofat.ipynb**: shows how to perform One Factor At the Time (OFAT) analysis.
- **/Code/sobol.ipynb**: shows how to perform Sobol sensitivity analysis.
...
It is now possible to progress the environment (a hundred timesteps), by doing the following:
```
for _ in range(100):
    env.step()
```
